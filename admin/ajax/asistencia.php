	...
	case 'listar':
		...
		while ($reg=$rspta->fetch_object()) {
			$data[]=array(
				...
				"4"=>$reg->departamento,
				"5"=>$reg->fecha_hora,
				"6"=>($reg->tipo=="Entrada")?'<span class="label bg-green">Entrada</span>':'<span class="label bg-red">Salida</span>',
				"7"=>$reg->fecha
				);
		}
    ...
    $codigo_persona=$_REQUEST["idcliente"]; 
		...
		while ($reg=$rspta->fetch_object()) {
			$data[]=array(
				...
				"2"=>$reg->nombre,
				"3"=>$reg->apellidos,
				"4"=>($reg->tipo=="Entrada")?'<span class="label bg-green">Entrada</span>':'<span class="label bg-red">Salida</span>',
				"5"=>$reg->fecha_hora,
				"6"=>$reg->codigo_persona
				);
		}
